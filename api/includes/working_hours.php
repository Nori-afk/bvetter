 ...
 *
 * Philippine holidays and the dates an admin marks closed are skipped too
 * (api/includes/clinic_calendar.php) -- call loadClinicCalendar($pdo) first
 * so the admin's dates are included.
 */
...
{
    for ($i = 0; $i < 90; $i++) {
        $isOpenDay = (int) $t->format('N') <= 5 && clinicClosedReason($t->format('Y-m-d')) === null;
        $open  = $t->setTime(BV_OFFICE_OPEN_HOUR, 0);
        $close = $t->setTime(BV_OFFICE_CLOSE_HOUR, 0);

        if ($isOpenDay && $t < $close) {
